...
lookback_months = config.get('lookback_months', 4)
...
for run_csvs_start in range(csvs_start, csvs_total_len-1, csvs_per_run):
    ...
    for i, days_path in enumerate(run_csv_paths):
        ...
        for first_day in range(lookback_days, max_day, lookback_days // 4): #first day is actually the first row and we look forward to the current day + look ahead days for prediction
            ...
            months = monthly_df.iloc[max(0, month_idx-lookback_months):month_idx, 1:].values #remove date col
            # if any of the indicators isnt ready, then skip
            if np.any(np.isnan(months)):
                continue
    ...
    if config.get('model_path'):
        ...
        daily = build_rnn_branch(daily_input, rnn_layers, rnn_units, rnn_type)
        weekly = build_rnn_branch(weekly_input, rnn_layers, rnn_units, rnn_type)
        monthly = build_rnn_branch(monthly_input, rnn_layers, rnn_units, rnn_type)
